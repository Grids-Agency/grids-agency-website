...
// Shift the resting position within a full-section canvas, so dragging never
// crosses a canvas edge at the boundary between the text and visual columns.
function SceneFraming({ horizontalOffset }) {
  const { camera, size } = useThree();
  useLayoutEffect(() => {
    if (!camera.isPerspectiveCamera || !size.width || !size.height) return;
    const offset = window.matchMedia('(min-width: 1024px)').matches ? horizontalOffset : 0;
    camera.setViewOffset(size.width, size.height, -size.width * offset, 0, size.width, size.height);
    ...
    <>
      <group position={[0, 4.65, 0]}>
        ...
        <RigidBody position={[2, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          <CuboidCollider args={[0.8 * cardScale, 1.125 * cardScale, 0.01 * cardScale]} />
          <group
