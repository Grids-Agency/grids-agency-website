...
uniform float uAurora, uIntensity;

// Smooth overlapping curtains, evaluated directly rather than on the dot grid.
vec3 aurora(vec2 uv) {
  float t = uTime;
  float x = uv.x;
  float nx = (x - 0.5) / 0.75;
  float arch = uPeak + nx * nx * uHeight * 0.4;
  float flow = sin(x * 5.4 + t * 0.32) * 0.075
             + sin(x * 10.5 - t * 0.24) * 0.028;
  float centerA = arch + flow;
  float centerB = arch + 0.11 + sin(x * 4.8 - t * 0.21 + 1.7) * 0.095;
  float centerC = arch - 0.12 + sin(x * 6.0 + t * 0.19 + 3.5) * 0.065;
  float width = 0.055 * uThick;
  float a = (uv.y - centerA) / width;
  float b = (uv.y - centerB) / (width * 1.3);
  float c = (uv.y - centerC) / (width * 0.9);
  float ribbonA = exp(-a * a);
  float ribbonB = exp(-b * b);
  float ribbonC = exp(-c * c);
  float halo = exp(-a * a * 0.12) * 0.24
             + exp(-b * b * 0.14) * 0.18;
  float palette = 0.5 + 0.5 * sin(x * 4.0 + t * 0.12);
  vec3 first = mix(uBase, uAccent, smoothstep(0.15, 0.9, palette));
  vec3 second = mix(uAccent, uHigh, 0.5 + 0.5 * sin(x * 3.5 - t * 0.15));
  vec3 third = mix(uHigh, uBase, smoothstep(0.0, 1.0, x));
  float weight = ribbonA + ribbonB * 0.8 + ribbonC * 0.65 + 0.001;
  vec3 ink = (first * ribbonA + second * ribbonB * 0.8 + third * ribbonC * 0.65) / weight;
  ink = mix(first, ink, clamp(weight, 0.0, 1.0));
  float energy = (weight + halo) * uIntensity;
  // Leave the headline and the lower section transition calm and legible.
  energy *= mix(0.12, 1.0, smoothstep(0.18, 0.43, uv.y));
  energy *= 1.0 - smoothstep(0.85, 1.04, uv.y);
  float coverage = 1.0 - exp(-energy * 1.15);
  vec3 color = mix(uBg, ink, min(coverage, 0.82));
  float dark = 1.0 - smoothstep(0.1, 0.7, dot(uBg, vec3(0.333333)));
  color += (first * ribbonA + third * ribbonC) * 0.13 * dark;
...
void main(){
  if (uAurora > 0.5) {
    ...
    uv.y = 1.0 - uv.y;
    gl_FragColor = vec4(aurora(uv), 1.0);
    return;
  }
...
export default function PredictiveArc(props: Props) {
    ...
    useEffect(() => {
      ...
      vRef.current = {
        ...
        intensity: clampN(num(intensity, 1), 0, 2),
        base: baseColor,
        accent: accentColor,
        high: highlight,
    ...
    }, [background, variant, intensity, baseColor, accentColor, highlight, density, dotSize, speed, arch, pointer, width, height])
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            gl.uniform1f(u("uAurora"), v.aurora ? 1 : 0)
    ...
    return (
        <div
            className="relative size-full overflow-hidden touch-none"
            style={{
                background,
