...
export default function ParticleWinding(props: Props) {
    const {
        style,
        dotColor = "#F4F1EA",
        accentColor = "#00FFE5",
    ...
    }, [dotColor, accentColor, speed, density, dotSize, spinTurns, pointer_.drag, pointer_.damping, ball_.spread, ball_.turn, ball_.tilt])
    ...
    useEffect(() => {
        ...
        if (!ctx) {
            console.error("ParticleWinding: 2D context unavailable")
            return
        }
        ...
        const resolveColor = (input: string, fallback: RGBA) => {
            const cached = colorCache.get(input)
            if (cached) return cached
            const resolved = input.replace(/var\((--[\w-]+)\)/g, (_, variable: string) =>
                window.getComputedStyle(canvas).getPropertyValue(variable).trim()
            )
            const color = css(parseColor(resolved, fallback))
        ...
        const render = (now: number) => {
            ...
            if (!media.matches) phase = (phase + (dt * (v.speed as number)) / PERIOD) % 1
            if (phase < 0) phase += 1
            ...
            const dotCol = resolveColor(v.dot as string, [244, 241, 234, 1])
            const accCol = resolveColor(v.acc as string, [232, 133, 60, 1])
            ...
            if (!media.matches && !document.hidden) raf = requestAnimationFrame(render)
        }
        ...
        const onDown = (e: PointerEvent) => {
            ...
            try {
                ...
                // Pointer capture may be unavailable when a pointer has already ended.
            }
        }
        ...
        const syncPlayback = () => {
            ...
            if (!document.hidden) render(last)
        }
        const themeObserver = new MutationObserver(syncPlayback)
    ...
    return (
        <div
            className="relative min-h-6 min-w-6 overflow-hidden"
            style={{
