...
export default function FeaturedWorkCarousel() {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "grid items-stretch gap-(--work-gap) [--work-gap:0.5rem] [--work-strip:2rem] md:[--work-gap:1.5rem] md:[--work-strip:3.5rem]",
          "[--work-card:calc(100cqw_-_2*var(--work-strip)_-_2*var(--work-gap))] md:[--work-card:calc((100cqw_-_var(--work-strip)_-_2*var(--work-gap))/2)]",
          "transition-[grid-template-columns] duration-900 ease-[cubic-bezier(0.22,1,0.36,1)] motion-reduce:transition-none",
          selection.active === 0 &&
            "grid-cols-[var(--work-card)_var(--work-strip)_var(--work-strip)]",
          selection.active === 1 &&
            "grid-cols-[var(--work-strip)_var(--work-card)_var(--work-strip)]",
          selection.active === 2 &&
            "grid-cols-[var(--work-strip)_var(--work-strip)_var(--work-card)]",
          collapsed === 0 &&
      ...
      >
        {projects.map(({ key, video }, index) => {
          ...
          return (
            <article key={key} className="relative min-w-0 overflow-hidden">
              {/* Fixed-width contents are clipped during the fold, never stretched. */}
              <div
                id={`${id}-${key}`}
                ...
                className={cn(
                  "flex h-full w-(--work-card) flex-col p-4 outline-none transition-[opacity,visibility] duration-400 motion-reduce:transition-none sm:p-6 md:p-8 lg:p-10",
                  mobileOpen
                    ? "visible opacity-100"
              ...
              >
                ...
                <span
                  aria-hidden="true"
                  className="text-xs font-medium tracking-wide whitespace-nowrap [writing-mode:vertical-rl] md:text-sm"
                >
                  {t(`${key}.title`)}
                </span>
                <ArrowRight
                  size={16}
                  aria-hidden="true"
                  className="shrink-0 transition-transform duration-300 group-hover:-rotate-45 motion-reduce:transition-none"
                />
              </button>
              <div
                aria-hidden="true"
