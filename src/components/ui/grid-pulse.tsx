...
export type GridPulseProps = Omit<React.ComponentPropsWithoutRef<"div">, "children"> & {
  /** Cell size in px, shared by the hairlines and lit cells. */
  cell?: number;
  /** Pointer reach, in cells. */
  reach?: number;
  /** Cells lit on each ambient beat. */
  ambient?: number;
  maxLit?: number;
  /** Text elements inside the grid's parent to protect from bright cells. */
  avoid?: string;
};
...
/** Grid Pulse by carolinaraulino, integrated from the supplied source.
 * A pointer-reactive spectrum, drawn on a single decorative canvas.
 */
export function GridPulse({
  ...
  React.useEffect(() => {
    ...
    // Read resolved colour through a pixel: computed styles can be oklch.
    ...
    const paint = () => {
      pending = 0;
      if (!at || !visible || document.hidden) return;
    ...
    const copy = new MutationObserver(() => {
      if (!recheck) recheck = requestAnimationFrame(() => {
        recheck = 0;
        measureText();
      });
    });
    ...
    // The Finder expands with a transform rather than a resize.
    el.parentElement?.addEventListener("transitionend", measureText);
    ...
    return () => {
      ...
      el.parentElement?.removeEventListener("transitionend", measureText);
    };
