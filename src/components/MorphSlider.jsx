...
// Adapted from React Bits MorphSlider: autoplay-only bento preview.
...
class MorphEngine {
  constructor(container, { items, startIndex, reducedMotion, getOptions, onIndexChange, dprCap }) {
    ...
    this.canvas.className = 'block size-full opacity-0';
  ...
  setActive(active) {
    this.active = active && !this.lost && !this.disposed;
  ...
  goTo(dir) {
    if (!this.active || this.animating || this.items.length < 2 || !this.loaded[this.current] || !this.loaded[this.wrap(this.current + dir)]) return;
    const opts = this.getOptions();
    if (!opts.loop) {
  ...
  destroy() {
    this.disposed = true;
    this.images.forEach(image => { image.onload = null; });
    cancelAnimationFrame(this.raf);
...
export default function MorphSlider({
  ...
  useEffect(() => {
    ...
    let failed = false;
    const options = {
      transition, duration, intensity, ease: 'power2.inOut',
      scale: 2.4, aberration: 0.08, drift: 0, overlayColor: '#000000', loop: true,
    };
    const syncPlayback = () => {
      clearInterval(timer);
      const active = visible && !document.hidden && !media.matches;
      if (active && !engine && !failed) {
        try {
          engine = new MorphEngine(container, {
            items, startIndex: 0, reducedMotion: false, dprCap: 1.5,
            getOptions: () => options,
          });
        } catch {
          // The local preview remains visible if WebGL is unavailable.
          failed = true;
          container.replaceChildren();
        }
      }
      engine?.setActive(active);
      if (active && engine && !engine.lost) {
        timer = setInterval(() => engine.next(), Math.max(autoplayDelay, duration + 1) * 1000);
      }
    };
  ...
  }, [items, transition, duration, intensity, autoplayDelay]);

  return (
    <div
      aria-hidden="true"
      className={`relative size-full overflow-hidden bg-background bg-cover bg-center ${className}`}
      style={{ backgroundImage: items[0] ? `url("${items[0].image}")` : undefined }}
    >
      <div ref={containerRef} className="absolute inset-0 motion-reduce:hidden" />
    </div>
  );
}
