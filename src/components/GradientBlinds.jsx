...
const MAX_COLORS = 8;
const DEFAULT_COLORS = ['#FF9FFC', '#5227FF'];
const hexToRGB = hex => {
...
`;
    ...
    const loop = t => {
      ...
      uniforms.iTime.value = paused ? 0 : elapsed;
      if (!paused && autoAnimate && t - lastPointerTime > 1800) {
        mouseTargetRef.current = [
          gl.drawingBufferWidth * (0.5 + Math.sin(elapsed * 0.18) * 0.18),
          gl.drawingBufferHeight * (0.52 + Math.sin(elapsed * 0.13) * 0.1)
        ];
      }
      if (mouseDampening > 0) {
