...
const CanvasWrapper = ({ children, active }) => (
  <Canvas dpr={[1, 1.25]} frameloop={active ? 'always' : 'demand'} className="relative h-full w-full" gl={{ antialias: false }} fallback={null}>
    {children}
  </Canvas>
);
...
const MergedPlanes = forwardRef(({ material, width, count, height }, ref) => {
  ...
  useFrame((_, delta) => {
    // Advance in seconds; cap the first frame after resuming to avoid a jump.
    mesh.current.material.uniforms.time.value += Math.min(delta, 0.05);
  });
