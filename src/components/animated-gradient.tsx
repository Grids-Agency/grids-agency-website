...
import { useRef, useEffect, useMemo, useSyncExternalStore, type CSSProperties } from "react";
...
export default function AnimatedGradient({
  ...
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver(() => {
      resize();
      if (params.speed === 0) animate(performance.now());
    });
    ...
    const animate = (time: number) => {
      ...
      gl.uniform1f(uniforms.u_pixelRatio, Math.min(window.devicePixelRatio || 1, MAX_DPR));
      ...
      if (params.speed > 0) frameIdRef.current = requestAnimationFrame(animate);
    };
